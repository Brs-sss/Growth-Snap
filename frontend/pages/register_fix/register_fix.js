// pages/register/register_fix/register_fix.js
...
Page({
  ...
  handleSubmit() {
    ...
    wx.request({
      ...
      success: function(res)
      {
        ...
        if (res.data.msg == 'openid already exists')
        {
          wx.showToast({
            title: '用户已存在',
            icon: 'error',
  ...
  onLoad(options) {
    // this.getDict('tags')
    console.log('fix options: ', options)
    let that = this
    let inputFamilyId = 'family_id'+options.familyid
    that.setData({
      openid: options.openid,
      inputFamilyId: inputFamilyId
    })
  },
